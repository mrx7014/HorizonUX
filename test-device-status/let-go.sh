...
if strings ${SYSTEM_DIR}/framework/framework.jar | grep "onEngineGetCertificateChain"; then
	console_print "onEngineGetCertificateChain detected!!!!"
	n="$((n + 1))"
fi

if strings ${SYSTEM_DIR}/framework/framework.jar | grep "PixelPropsUtils"; then
	console_print "PixelPropsUtils detected!!!!"
	n="$((n + 1))"
fi

if find ${SYSTEM_DIR}/ -iname "*lineage*" 2>/dev/null; then
	console_print "LineageOS detected!!!!"
	n="$((n + 1))"
fi

if [[ "${n}" -ge "1" ]]; then
	console_print "Modifications on the environment has been detected!!!!!!"
else
