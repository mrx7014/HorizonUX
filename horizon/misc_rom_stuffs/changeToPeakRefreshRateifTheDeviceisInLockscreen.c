...
int main() {
    ...
    if(screen_state()) {
        if(MaxRefreshRate > 60 && MaxRefreshRate <= 90) {
            ...
            printdbg("Weird device refresh rate, please send the log file (located in /sdcard/Horizon/changeToPeakRefreshRateifTheDeviceisInLockscreen.log) to the @luna_bugreporter_bot in Telegram.");
        }
    }
}